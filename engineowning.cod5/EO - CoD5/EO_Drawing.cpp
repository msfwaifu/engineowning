...
fontInfo_t * ( *R_RegisterFont )( char* FontName, signed int Unknown )	 = ( fontInfo_t * (__cdecl*) ( char *, signed int ) )0x6BAD40;
qhandle_t    ( *R_RegisterShader )( char* ShaderName, signed int Unknown ) = ( int (__cdecl*)( char *, signed int ) ) 0x6BBA70;
SIZE_T (*CG_DrawStretchPic)(float x, float y, float w, float h, float s1, float t1, float s2, float t2, float* color, qhandle_t hShader) = ( SIZE_T (__cdecl *)(float, float, float, float, float, float, float, float, vec3_t, qhandle_t  ))0x6BD830;
...
{
	CG_DrawStretchPic(x,y,w,h,0,0,1,1,pColor,Shader);
}
